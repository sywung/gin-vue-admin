...
import (
	...
	"github.com/sywung/gin-vue-admin/server/resource/autocode_template/subcontract"

	"github.com/sywung/gin-vue-admin/server/global"
	"github.com/sywung/gin-vue-admin/server/model/system"
	"github.com/sywung/gin-vue-admin/server/utils"
...
func Init(Package string) {
	...
	packageInjectionMap = map[string]astInjectionMeta{
		packageServiceName: {
			...
			importCodeF:  "github.com/sywung/gin-vue-admin/server/%s/%s",
			packageNameF: "%s",
...
func (autoCodeService *AutoCodeService) CreateTemp(autoCode system.AutoCodeStruct, ids ...uint) (err error) {
	...
	if autoCode.AutoMoveFile { // 判断是否需要自动转移
		...
		var routePath = filepath.Join(global.GVA_CONFIG.AutoCode.Root,
			global.GVA_CONFIG.AutoCode.Server, global.GVA_CONFIG.AutoCode.SInitialize, "router.go")
		var imporStr = fmt.Sprintf("github.com/sywung/gin-vue-admin/server/model/%s", autoCode.Package)
